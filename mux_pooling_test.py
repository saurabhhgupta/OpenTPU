...
def mux_pooling(start, vecs, vecs_length, nvecs, matrix_size, mat_size_reg):
	...
	with conditional_assignment:
		with start:
			...
			constant_val.next |= 0x80000000
			mask.next |= barrel_shifter_v2(mask, 1, 0, mat_size_reg)
		with init:
			busy.next |= 1
			init.next |= 0
	...
	return output_vecs, mux_control, counter, busy, init, done, mask
...
vecs_length = Register(4, 'vecs_length')
mat_size_reg = Register(4, 'mat_size_reg')
nvecs.next <<= 6
mat_size = 8
mat_size_reg.next <<= 8
pool_size = 2
...
output_vecs, mux_control, counter, busy, init, done, mask = mux_pooling(start, reg_vec, vecs_length, nvecs, mat_size, mat_size_reg)
...
print('--- Simulation ---')
sim_trace.render_trace(symbol_len=12, segment_size=35)