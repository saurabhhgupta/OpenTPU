		...
		with pooling == 1: #start line pooling phase
			with busy == 0:
				...
				with otherwise:
					shifting.next |= shifting + 1
			with otherwise:
				pooled_value = line_pool(line_pool_lists[0], busy)
				check = line_pool_lists[0]
	return single_list, pooled_value, shifting, pooling, setup, pool_count, busy, check #needs more outputs.
...
'''
# def normalize(start, vecs, nvecs, matrix_size):
# 	line_pool_lists = [[Register(32) for i in range(0, matrix_size)] for j in range(0, matrix_size)]
...
# 				offset.next |= 0
# 				nrml.next |= 1
# 				offset_amt.next |= counter
# 			with counter == 23: #offset = max = 24
# 				offset.next |= 0
# 				nrml.next |= 1
# 				offset_amt.next |= 0
# 			with otherwise:
...
# 				for reg in range(0, matrix_size):
# 					output_list[reg].next |= line_pool_lists[0][reg][offset_amt:offset_amt+8]
# 					line_pool_lists[-1][reg].next |= 0x80000000
# 	return output_list, done
...
# out_list, done = normalize(start, reg_vec, nvecs, mat_size)
# for count, i in enumerate(out_list):
# 	probe(i, 'output_list_{}'.format(count))
# probe(done, 'done')
...
for index,reg in enumerate(reg_vec):
	reg.next <<= inputs[index]
	output_orig[index] <<= reg
single_list, pooled_value, shifting_wire, pooling_wire, setup_wire, pool_count, busy, check = intermediate_pool(start, reg_vec, nvecs, vecs_length, mat_size, pool_size)
probe(shifting_wire, 'shifting_wire')
probe(pooling_wire, 'pooling_wire')
probe(setup_wire, 'setup_wire')
probe(pool_count, 'pool_count')
probe(busy, 'busy')
